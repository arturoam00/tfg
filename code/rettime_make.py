...
rmin = .15 
rmax = .8  #when working with tau0 rho can't reach 1 (.95)
smin = .15
smax = .8  #when working with r / d <<< sigma = 1 means dispersal does nothing and it's very slow
...
with open("values_tau.npy", "wb") as f:
    np.save(f, a)
    np.save(f, constants)
