...
img = ax.imshow(regimes_array, origin = "lower", extent=[dmin, dmax, lmin, lmax], \
	aspect = (dmax - dmin) / (lmax - lmin), cmap = "terrain")
...
line, = ax.plot(dis_values_log, np.log10(y1), "-m")
line2, = ax.plot(dis_values_log, np.log10(y2), "-m")
pl.ylim(lmin, lmax)
...
# 	aspect = (dmax - dmin) / (gmax - gmin), cmap = "terrain")
...
# line, = ax.plot(x1, g_values, "-m")
# line2, = ax.plot(x2, g_values, "-m")
# pl.ylim(gmin, gmax)
# pl.xlim(dmin, dmax)
...
# 	aspect = (dmax - dmin) / (rmax - rmin), cmap = "terrain")
...
# line, = ax.plot(x1, rho_values, "-m")
# line2, = ax.plot(x2, rho_values, "-m")
# pl.ylim(rmin, rmax )
# pl.xlim(dmin, dmax)
